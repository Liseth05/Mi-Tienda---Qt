...
void Principal::on_cmdAgregar_released()
{
    ...
    if (nombre.isEmpty()){
        QMessageBox::warning(this, "AVISO","Ingresar nombre del Cliente");
        return;
    }
    else if (cedula.isEmpty()) {
        QMessageBox::warning(this, "AVISO","Ingresar cédula del Cliente");
        return;
    }
...
void Principal::on_inCedula_textChanged(const QString &arg1)
{
}

void Principal::on_inCedula_selectionChanged()
{
}

void Principal::on_inCedula_editingFinished()
{
    bool validadorCedula;

    ValidarCedula ob;
    QString cedula = ui->inCedula->text();
    string cedula1 = cedula.toStdString();
    validadorCedula = ob.verificarCedula(cedula1);
    if(validadorCedula){
        ui->cmdAgregar->setEnabled(true);
        return;
    } else {
        QMessageBox::warning(this, "AVISO",
                             "Cédula Incorrecta");
        ui->cmdAgregar->setEnabled(false);
        return;
    }
